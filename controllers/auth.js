...
export const addToLikedMovies = async (req, res) => {
  try {
    const { email, movieData } = req.body;
    ...
    if (user) {
      ...
      if (!isMovieAlreadyLiked) {
        ...
        await User.findByIdAndUpdate(
          user._id,
          {
            likedMovies: [...user.likedMovies, movieData],
          },
          {
            new: true,
          }
...
// Removing movie from the liked movies list
export const removeFromLikedMovies = async (req, res) => {
  try {
    ...
    // check if the movie is in liked movies list
    ...
    if (movieIndex > -1) {
      ...
      res
        .status(400)
        .json({ message: "Movie is not in the liked Movies list" });
    }
  } catch (err) {
    res
      .status(400)
